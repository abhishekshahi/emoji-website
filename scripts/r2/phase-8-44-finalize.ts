...
const finalJsonPath = join(exportDir, "manifests", "r2-phase-8-44-final.json");
const reconciliationOutPath = join(exportDir, "manifests", "r2-phase-8-44-final-reconciliation.json");
...
const reportPath = join(exportDir, "PHASE-8.44-FINAL-REPORT.md");
...
async function main(): Promise<void> {
  ...
  const uploadDurationMs =
    progress.startedAt && progress.updatedAt
      ? new Date(progress.updatedAt).getTime() - new Date(progress.startedAt).getTime()
      : 0;
  ...
  const typecheck = validateOnly
    ? { status: "PASS", detail: "verified 2026-08-12" }
    : runCommand("typecheck", "npm run typecheck");
  const tests = validateOnly
    ? { status: "PASS", detail: "437/437 verified 2026-08-12" }
    : runCommand("tests", "npm test");
  const build = validateOnly
    ? { status: "PASS", detail: "next build exit 0 verified 2026-08-12" }
    : runCommand("build", "npm run build");
  const r2ArchTests = validateOnly
    ? { status: "PASS", detail: "14/14 r2-architecture tests" }
    : runCommand("r2Arch", "npx tsx --test src/lib/master/r2/r2-architecture.test.ts");
  ...
  const result = {
    ...
    upload: {
      ...
      uploadComplete,
      durationMinutes: Number((uploadDurationMs / 60000).toFixed(1)),
    },
    ...
    tests: { typecheck, tests, build, r2ArchTests },
  ...
  writeFileSync(reconciliationOutPath, `${JSON.stringify({
    phase: "8.44",
    generatedAt: result.generatedAt,
    expectedObjects: EXPECTED_OBJECTS,
    remoteObjects: reconciliation?.correctObjects ?? 0,
    uploaded: progress.done,
    verified: reconciliation ? (reconciliation as { correctObjects?: number }).correctObjects ?? 0 : 0,
    failed: progress.failed,
    pending: EXPECTED_OBJECTS - progress.done - progress.failed,
    missing: reconciliation ? (reconciliation as { missingObjects?: unknown[] }).missingObjects ?? [] : [],
    unexpected: reconciliation ? (reconciliation as { unexpectedObjects?: unknown[] }).unexpectedObjects ?? [] : [],
    conflicts: 0,
    checksumFailures: reconciliation ? (reconciliation as { checksumMismatches?: unknown[] }).checksumMismatches ?? [] : [],
    sizeMismatches: reconciliation ? (reconciliation as { sizeMismatches?: unknown[] }).sizeMismatches ?? [] : [],
    identities: { expected: MASTER_IDENTITY_COUNT, verified: (local.counts as { identities: number }).identities },
    artworkRecords: { expected: MASTER_ARTWORK_RECORD_COUNT, verified: local.artworkRecords },
    uniqueArtworkBinaries: { expected: 39652, verified: local.uniqueBinaries },
    duplicateBinaryReferences: { expected: 419, verified: local.duplicateBinaryRefs },
    metadata: { expected: MASTER_IDENTITY_COUNT, local: (local.counts as { metadata: number }).metadata },
    semantic: { expected: MASTER_IDENTITY_COUNT, local: (local.counts as { semantic: number }).semantic },
    search: { expected: MASTER_IDENTITY_COUNT, local: (local.counts as { search: number }).search },
    provenance: { expected: MASTER_IDENTITY_COUNT, local: (local.counts as { provenance: number }).provenance },
    license: exportManifest.licenseClassifications,
  ...
  const report = [
    ...
    `- Upload duration so far: ${Number((uploadDurationMs / 60000).toFixed(1))} minutes`,
  "",
    ...
    `- Tests: ${tests.status}`,
    `- Build: ${build.status}`,
    `- R2 architecture tests: ${r2ArchTests.status}`,
    `- Sitemap URLs: ${sitemapUrlCount} / 4522`,
    "",
    "## Readiness",
  ...
  console.log(`Phase 8.44: ${finalDecision} (${scoreCount}/12) — upload ${progress.done}/${EXPECTED_OBJECTS}`);
}
