...
const VERIFY_CONCURRENCY = Number(process.env.R2_VERIFY_CONCURRENCY ?? "12");
...
const checkpointPath = join(exportDir, "manifests", "r2-phase-8-50-checkpoint.json");
...
function loadCheckpoint(): VerifyCheckpoint | null {
  if (!existsSync(checkpointPath) || process.env.R2_RESET_CHECKPOINT === "1") {
    return null;
  }
  const checkpoint = JSON.parse(readFileSync(checkpointPath, "utf8")) as VerifyCheckpoint;
  if (!Array.isArray(checkpoint.verifiedKeys)) return null;
  log(`Loaded checksum checkpoint: ${checkpoint.verifiedKeys.length} verified objects`);
  return checkpoint;
}

function saveCheckpoint(stats: VerifyStats): void {
  const checkpoint: VerifyCheckpoint = {
    updatedAt: new Date().toISOString(),
    verifiedKeys: stats.verifiedKeys,
    checksumMismatch: stats.checksumMismatch,
    sizeMismatches: stats.sizeMismatches,
    missing: stats.missing,
    unableToVerify: stats.unableToVerify,
    retries: stats.retries,
  };
  writeFileSync(checkpointPath, `${JSON.stringify(checkpoint)}\n`, "utf8");
}

function statsFromCheckpoint(checkpoint: VerifyCheckpoint | null): VerifyStats {
  if (!checkpoint) {
    return {
      verified: 0,
      verifiedKeys: [],
  ...
  if (checkpoint.unableToVerify.length > 0) {
    log(`Retrying ${checkpoint.unableToVerify.length} previously unable-to-verify objects`);
  }
  return {
    verified: checkpoint.verifiedKeys.length,
    verifiedKeys: [...checkpoint.verifiedKeys],
    checksumMismatch: [...checkpoint.checksumMismatch],
    sizeMismatches: [...checkpoint.sizeMismatches],
...
): Promise<{ augmented: number; bindingMissing: string[] }> {
  const dotEntries = entries.filter(
    (entry) => entry.objectKey.includes("..") && !remoteMap.has(entry.objectKey),
  );
  if (dotEntries.length === 0) {
    return { augmented: 0, bindingMissing: [] };
  }

  log(
    `Probing ${dotEntries.length} dot-key objects via MASTER_R2 binding (REST list cannot enumerate them)`,
  );
  const bindingMissing: string[] = [];
  let augmented = 0;
  for (const entry of dotEntries) {
    const bytes = await downloadViaPlatformProxy(entry.objectKey);
    if (!bytes) {
      bindingMissing.push(entry.objectKey);
      continue;
    }
    remoteMap.set(entry.objectKey, bytes.length);
    augmented += 1;
  }
  log(`Binding probe complete: ${augmented}/${dotEntries.length} dot-keys present`);
  return { augmented, bindingMissing };
}
...
async function verifyChecksums(
  ...
  const stats = statsFromCheckpoint(checkpoint);
  ...
  const pendingEntries = entries.filter((entry) => !completedKeys.has(entry.objectKey));
  log(
    `Checksum queue: ${pendingEntries.length} pending (${completedKeys.size} already checkpointed), concurrency=${concurrency}`,
  );

  let idx = 0;
  let active = 0;
  const started = Date.now();
  let lastCheckpointAt = completedKeys.size;

  await new Promise<void>((resolve) => {
    const next = (): void => {
      while (active < concurrency && idx < pendingEntries.length) {
        ...
        void (async () => {
          ...
          if (remoteSha256(entry.objectKey, remote) !== entry.sha256) {
            ...
            stats.verified += 1;
            stats.verifiedKeys.push(entry.objectKey);
          }
        })()
          .catch((error: unknown) => {
            const message = error instanceof Error ? error.message : String(error);
            stats.unableToVerify.push(entry.objectKey);
            log(`Checksum worker error for ${entry.objectKey}: ${message}`);
          })
          .finally(() => {
            const done =
              stats.verified +
              stats.checksumMismatch.length +
              stats.sizeMismatches.length +
              stats.missing.length +
              stats.unableToVerify.length;
            if (done % 500 === 0 || done === entries.length) {
              const elapsed = ((Date.now() - started) / 1000).toFixed(1);
              const rate = done > completedKeys.size
                ? ((done - completedKeys.size) / ((Date.now() - started) / 1000)).toFixed(1)
                : "0";
              log(`Checksum verify: ${done}/${entries.length} (${rate}/s, ${elapsed}s elapsed)`);
            }
            if (done - lastCheckpointAt >= 500 || done === entries.length) {
              saveCheckpoint(stats);
              lastCheckpointAt = done;
            }
            active -= 1;
            if (idx >= pendingEntries.length && active === 0) resolve();
            else next();
          });
      }
    };
...
async function main(): Promise<void> {
  ...
  if (existsSync(remoteListCachePath) && process.env.R2_SKIP_REMOTE_LIST === "1") {
    ...
    log(`Loaded remote list cache: ${remoteMap.size} objects`);
  } else {
  ...
  const scores = {
    objectCount: canonicalPresent === EXPECTED_OBJECTS,
    ...
    sizeMismatchesZero: sizeMismatches.length === 0 && checksumStats.sizeMismatches.length === 0,
    checksumsVerified:
  ...
  const verification = {
    ...
    method:
      "Cloudflare R2 API list + MASTER_R2 binding probe for dot-keys + HTTP/binding SHA-256 (JSON/md/txt strip trailing newline)",
    ...
    sizeMismatches: sizeMismatches.length + checksumStats.sizeMismatches.length,
  ...
  const reconciliation = {
    ...
    unexpectedObjects,
    conflicts: listConflicts,
    sizeMismatches: [...new Set([...sizeMismatches, ...checksumStats.sizeMismatches])],
  ...
  if (existsSync(checkpointPath)) {
    ...
    log("Removed checksum checkpoint after successful verification");
  }
  ...
  const report = [
    ...
    "## Object Count",
    `- Expected: ${EXPECTED_OBJECTS}`,
    `- REST list: ${restListCount}`,
    `- Binding-augmented dot-keys: ${dotKeyProbe.augmented}`,
    `- Canonical present: ${canonicalPresent}`,
    `- Remote total (incl. unexpected): ${remoteMap.size}`,
    `- Missing: ${missingObjects.length}`,
    `- Unexpected: ${unexpectedObjects.length}`,
    `- Conflicts: ${listConflicts.length}`,
    "",
    "## Size Verification",
    `- Size mismatches: ${verification.sizeMismatches}`,
    "",
  ...
  log(
    `Phase 8.50 complete: ${finalDecision} (${passCount}/${totalCriteria}) — canonical ${canonicalPresent}/${EXPECTED_OBJECTS}, verified ${checksumStats.verified}/${EXPECTED_OBJECTS}`,
  );
}
...
main().catch(async (error: unknown) => {
  ...
  const message = error instanceof Error ? error.message : String(error);
  console.error(`Phase 8.50 verification failed: ${message}`);
...
process.on("unhandledRejection", (reason) => {
  console.error("Unhandled rejection in phase-8-50-verify:", reason);
  process.exitCode = 1;
});
