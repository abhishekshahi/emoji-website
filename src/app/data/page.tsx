...
import { getRightsDashboardStats } from "@/lib/master/public/asset-rights";
...
export default function DataDownloadsPage() {
  ...
  const rights = getRightsDashboardStats();
  ...
  return (
    <div className="page-shell space-y-8">
      ...
      <section className="card-surface space-y-4 p-6">
        <h2 className="text-xl font-semibold">License verification dashboard</h2>
          ...
          <table className="w-full min-w-[640px] text-left text-sm">
            <thead>
              <tr className="border-b border-border text-muted">
                ...
                <th className="py-2 pr-4">Restricted</th>
                <th className="py-2 pr-4">Public</th>
                <th className="py-2">Download</th>
              </tr>
            </thead>
            <tbody>
              {rights.providers.map((row) => (
                <tr key={row.provider} className="border-b border-border/60">
                  <td className="py-2 pr-4">{row.provider}</td>
                  <td className="py-2 pr-4">{row.assetCount.toLocaleString()}</td>
                  <td className="py-2 pr-4">{row.verified}</td>
                  <td className="py-2 pr-4">{row.pending + row.partial}</td>
                  <td className="py-2 pr-4">{row.restricted}</td>
                  <td className="py-2 pr-4">{row.publicServe}</td>
                  <td className="py-2">{row.downloadable}</td>
                </tr>
              ))}
            </tbody>
          </table>
