...
/** Reconcile RAW → AGGREGATED → NORMALIZED with zero silent loss. */
export function runNoLossAudit(input: {
  ...
  for (const agg of aggregated) {
    for (const ref of agg.source_refs) {
      referencedIds.add(ref.source_item_id);
    }
  }
  ...
  const reviewItems = validation.filter((v) => v.classification === "REVIEW").length;
  const invalidItems = validation.filter((v) => v.classification === "INVALID").length;
